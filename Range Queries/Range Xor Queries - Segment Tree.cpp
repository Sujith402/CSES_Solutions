...
class SegTree {
    ...
    void update(int node) {
        XOR[node] = XOR[2 * node] ^ delta[2 * node] ^ XOR[2 * node + 1] ^ delta[2 * node + 1];
    }

    int Xor(int node, int l, int r) {
        if (r < lo[node] || l > hi[node])
            return 0;

        if (lo[node] >= l && hi[node] <= r)
            return XOR[node] ^ delta[node];

        prop(node);

        int XorLeft = Xor(2 * node, l, r);
        int XorRight = Xor(2 * node + 1, l, r);
